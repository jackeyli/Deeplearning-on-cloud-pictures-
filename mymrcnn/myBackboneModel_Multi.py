...
def dense_graph_simple_long(input_image,stage4=True,train_bn=True,net_name="dense_res_l"):
    # Stage 1
    x = KL.Conv2D(32, (7, 7), strides=(2, 2), name=net_name + 'conv1', use_bias=True,padding="same")(input_image)
    x = BatchNorm(name= net_name + 'bn_conv1')(x, training=train_bn)
    x = KL.Activation('relu')(x)
    C1 = x = KL.MaxPooling2D((7, 7), strides=(2, 2), padding="same")(x)
    # Stage 2
    x = connectedConv(x, 7, [32, 32, 128], stage=2, block='a', strides=(1, 1), train_bn=train_bn,net_name=net_name)
    x = KL.Dropout(rate=0.2,name=net_name + "dense_dropout2")(x)
    #x = connectedIdentity(x, 7, [32, 32, 128], stage=2, block='b', strides=(1, 1), train_bn=train_bn,net_name=net_name)
    C2 = x = KL.MaxPooling2D((3, 3), strides=(2, 2), padding="same")(x)
    # Stage 3
    x = connectedConv(x, 7, [64, 64, 256], stage=3, block='a', train_bn=train_bn,net_name=net_name)
    C3 = x = KL.Dropout(rate=0.2,name=net_name +"dense_dropout3")(x)
    #C3 = x = connectedIdentity(x, 7, [64, 64, 256], stage=3, block='b', strides=(1, 1), train_bn=train_bn,net_name=net_name)
    # Stage 4
    if stage4:
        x = connectedConv(x, 7, [128, 128, 512], stage=4, block='a', train_bn=train_bn,net_name=net_name)
        C4 = x = KL.Dropout(rate=0.2,name= net_name + "dense_dropout5")(x)
        #C4 = x = connectedIdentity(x, 7, [128, 128, 512], stage=4, block='b', train_bn=train_bn,net_name=net_name)
    else:
        C4 = None
    return [C1, C2, C3,C4]
...
def msk_class_loss_graph(y_true, y_pred):
    return  K.mean(-K.square(1. - y_pred) * y_true * K.log(y_pred + 1e-9) \
            - K.square(y_pred) * (1. - y_true) * K.log(1. - y_pred + 1e-9))
class MyBackboneModel:
    ...
    def buildModel(self,mode,config):
        ...
        L1, L2, L3,L4 = dense_graph_simple_long(input_image,
        ...
        P_L_all = KL.Conv2D(512, (7, 7),name='pre_class_conv_l_1',padding="same")(L4)
        classifierLayer_l = KL.Conv2D(config.NUM_CLASSES,(12,18),name='classifier_conv_l',padding="valid")(P_L_all)
        mrcnn_class_logits_l = KL.Lambda(lambda x:K.squeeze(K.squeeze(x,axis=2),axis=1))(classifierLayer_l)
        mrcnn_class_logits_l = KL.Activation("sigmoid")(classifierLayer_l)
    ...
    def compile(self,learning_rate, momentum):
        ...
        #add binary acc
            ...
            catlacc = K.mean(K.equal(class_input, K.round(class_pred_l)))
